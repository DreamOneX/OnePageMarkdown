...
class MarkdownVisualTransformation : VisualTransformation {
    ...
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        while (s < src.length) {
            val m = listOf(bold, italic, code)
                .mapNotNull { it.find(src, s) }
            ...
            repeat(m.value.takeWhile { it == '*' || it == '`' }.length) {
                o2t[s++] = t2o.size
            }

            val content = m.groupValues[1]
            val style = when (m.regex) {
            ...
            repeat(m.value.takeLastWhile { it == '*' || it == '`' }.length) {
                o2t[s++] = t2o.size
            }
        }
